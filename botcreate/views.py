    ...
    if from_date=="":
        ...
        if selected_options:
            if "All" in selected_options:
                ...
                Bots = Bot.objects.filter(Q(Startdate__range=(from_date, to_date)) | Q(enhancestartdate__range=(from_date, to_date))).order_by('Botno')
            else:
                #Bots = Bot.objects.filter(Startdate__range=[from_date, to_date],Botstatus__in=selected_options).order_by('Botno')
                Bots = Bot.objects.filter(Q(Startdate__range=(from_date, to_date)) | Q(enhancestartdate__range=(from_date, to_date)),Botstatus__in=selected_options).order_by('Botno')
        else:
            Bots = Bot.objects.filter(Q(Startdate__range=(from_date, to_date)) | Q(enhancestartdate__range=(from_date, to_date)),Botstatus__in=selected_options).order_by('Botno')
            #Bots = Bot.objects.filter(Startdate__range=[from_date, to_date]).order_by('Botno')
...
def downloadreport(request):
    ...
    if from_date=="":
        ...
        if len(selected_options[0])>0:
            if "All" in selected_options:
                ...
                my_queryset = Bot.objects.filter(Q(Startdate__range=(from_date, to_date)) | Q(enhancestartdate__range=(from_date, to_date)),
                    Botstatus__in=selected_options).order_by('Botno')
        else:
            #my_queryset = Bot.objects.all().filter(Startdate__range=[from_date, to_date]).order_by('Botno')
            my_queryset = Bot.objects.filter(Q(Startdate__range=(from_date, to_date)) | Q(
    ...
    for obj in my_queryset:
        ...
        ws.append([obj.Botno,obj.Botname,obj.Process,obj.Subprocess,obj.Spocname,obj.Requestormail,obj.Teamleadmail,obj.Developermail,obj.Technologyused,creationdate,startdate,enddate,enhancestartdate,enhanceenddate,obj.Manualtimespend,obj.Automationtimespend,obj.Totaltimesaved,obj.Totaldaysaved,obj.Kaizenawardstatus,obj.Kaizenawardyear,obj.Botstatus,obj.Botdesc])
